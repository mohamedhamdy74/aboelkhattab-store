...
export default function ProductForm({ categories }: { categories: Category[] }) {
  ...
  const [variants, setVariants] = useState<Variant[]>([
    { sizeAr: "حجم واحد", sizeEn: "Standard", price: "", oldPrice: "", stock: "0", isDefault: true },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!imageFile) {
      alert("يرجى اختيار صورة المنتج الرئيسية");
      setLoading(false);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-24">
      ...
      <div className="rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/60 p-8 shadow-sm space-y-6">
        <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-100 border-b border-zinc-100 dark:border-zinc-800 pb-3">
          البيانات الأساسية
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-2">
            ...
            <input required name="nameAr" type="text" className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition text-sm" />
          </div>
          <div className="space-y-2">
            ...
            <input required name="nameEn" type="text" dir="ltr" className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition text-sm" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <select required name="categoryId" className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition text-sm cursor-pointer">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">صورة المنتج الرئيسية <span className="text-red-500">*</span></label>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                <button type="button" onClick={() => imageRef.current?.click()} className="px-4 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                  <ImagePlus className="w-4 h-4 inline ml-1" /> اختيار صورة
                </button>
                {imagePreview && (
                  <button type="button" onClick={() => { setImagePreview(null); if (imageRef.current) imageRef.current.value = ""; }} className="mr-2 px-3 py-2 rounded-xl border border-red-200 text-xs font-medium text-red-500 hover:bg-red-50 transition">
                    <X className="w-3.5 h-3.5 inline ml-1" />حذف
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              {galleryPreviews.map((url, idx) => (
                <div key={idx} className="relative w-20 h-20 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700">
                  ...
                  <button type="button" onClick={() => removeGalleryImage(idx)} className="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full shadow-md hover:bg-red-600 transition">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-2">
            ...
            <textarea name="descriptionAr" rows={3} className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition text-sm" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">وصف المنتج (إنجليزي)</label>
            <textarea name="descriptionEn" rows={3} dir="ltr" className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition text-sm" />
          </div>
        </div>
      </div>
            ...
            <div key={index} className={`p-5 rounded-2xl border transition ${v.isDefault ? "border-indigo-300 dark:border-indigo-500/50 bg-indigo-50/30 dark:bg-indigo-900/10" : "border-zinc-200 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-800/30"}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="space-y-1">
                  ...
                  <input required value={v.sizeAr} onChange={e => updateVariant(index, "sizeAr", e.target.value)} type="text" placeholder="تولة" className="w-full px-3 py-2.5 text-sm rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-400/40" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500">اسم الحجم (إنجليزي) *</label>
                  <input required value={v.sizeEn} onChange={e => updateVariant(index, "sizeEn", e.target.value)} type="text" dir="ltr" placeholder="Tola" className="w-full px-3 py-2.5 text-sm rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-400/40" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500">السعر (ج.م) *</label>
                  <input required value={v.price} onChange={e => updateVariant(index, "price", e.target.value)} type="number" step="0.01" min="0" placeholder="150" className="w-full px-3 py-2.5 text-sm font-bold text-indigo-600 dark:text-indigo-400 rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-indigo-400/40" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-zinc-500">السعر القديم (اختياري)</label>
                  <input value={v.oldPrice} onChange={e => updateVariant(index, "oldPrice", e.target.value)} type="number" step="0.01" min="0" placeholder="200" className="w-full px-3 py-2.5 text-sm text-zinc-500 rounded-xl border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-300" />
                </div>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-t border-zinc-200 dark:border-zinc-800 z-50 flex justify-end gap-3 md:pr-72">
        ...
        <button disabled={loading} type="submit" className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-wait text-white font-bold rounded-xl shadow-lg shadow-indigo-500/25 transition flex items-center gap-2 text-sm">
          {loading ? (
            <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> جاري الرفع والحفظ...</>
          ) : (
